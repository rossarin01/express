...
@section('content')
            ...
            <div
                ...
                <div id="content-to-be-converted" style="position: relative; width:800px;  height: 1000px;">
                    ...
                    @for ($i = 1; $i <= 5; $i++)
                        @php
                            ...
                            $receiptDescription = $receipt->{'receipt_new_description_' . $i};
                        @endphp
                ...
                @if (optional(optional(optional($receipt)->invoice)->job)->sells)
                    @foreach (optional(optional(optional($receipt)->invoice)->job)->sells as $sell)
                        <div
                            ...
                            @if ($sell->rate == 1)
                                {{ number_format($sell->value * optional(optional($receipt->invoice)->job)->sell_rate_value, 2) }}
                            @else
                ...
                @endif
                    ...
                    <div
                        ...
                        {{ number_format(optional(optional($receipt->invoice)->job)->sell_grand_total - optional(optional($receipt->invoice)->job)->sell_tax_3 - optional(optional($receipt->invoice)->job)->sell_tax_1, 2) }}
                    </div>

                    <div
                        style="font-size: 12px; position:absolute; top:785px; left:300px; font-family: 'Noto Sans Thai', sans-serif; vertical-align: top;">
                        ({{ baht_text(optional(optional($receipt->invoice)->job)->sell_grand_total - optional(optional($receipt->invoice)->job)->sell_tax_3 - optional(optional($receipt->invoice)->job)->sell_tax_1) }})
                    </div>

                    @if ($receipt->payment_method == 'cash')
                    <div
                        style="font-size: 12px; position:absolute; top:820px; left:100px; font-family: 'Noto Sans Thai', sans-serif; vertical-align: top;">
                        เงินสด
                    </div>
                    @endif
                    ...
                    <div
                        style="font-size: 12px; position:absolute; top:820px; left:150px; font-family: 'Noto Sans Thai', sans-serif; vertical-align: top;">
                        เงินโอน
                    </div>
                    @endif
                    ...
                    <div
                        style="font-size: 12px; position:absolute; top:850px; left:100px; font-family: 'Noto Sans Thai', sans-serif; vertical-align: top;">
                        เช็คธนาคาร : {{ $receipt->bank }} {{ $receipt->branch }} {{ $receipt->bank_number }} {{  \Carbon\Carbon::parse($receipt->transaction_date)->format('d/m/Y') }}
                    </div>
                    @endif
    ...
    <script>
        ...
        document.getElementById('close-pdf-btn').addEventListener('click', function() {
            ...
            window.location.href = '{{ route('receipt.vat.index') }}'; // เปลี่ยนเป็น URL ของหน้า index
        });
    </script>
