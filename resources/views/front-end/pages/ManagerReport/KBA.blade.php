...
@section('content')
    ...
    <script>
        ...
        document.getElementById('customTable').addEventListener('click', function() {
            ...
            var url = "{{ route('manager.kba', ['start' => ':start', 'end' => ':end']) }}";
        ...
        <div id="content-to-be-converted">
            ...
            <div style="width: 100%; margin-bottom: 20px;" class="font">
                <div style="width: 100%; text-align: center;">
                    <strong class="font" style="font-size: 20px;">KBA Report</strong>
                </div>
            ...
            @foreach ($agents as $agent)
                ...
                <div style="width: 100%; margin-bottom: 10px;" class="font">
                    <table style="width: 100%; border-collapse: collapse; border: none;">
                        <tr>
                            <td style="text-align: left; border: none;">
                                <strong class="font">AGENT : {{ $agent->name }}</strong>
                            </td>
                            <td style="text-align: right; border: none;">
                                <strong class="font">Contact : {{ $agent->contact }}</strong>
                            </td>
                        </tr>
                    </table>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="table table-bordered table-striped table-hover" style="text-align: center;"
                        ...
                        <tbody>
                            ...
                            @endphp
                            @foreach ($jobs as $job)
                                {{-- Check if the job belongs to the current agent --}}
                                @if ($job->draft && $job->draft->agent_id == $agent->id)
                                    <tr class="main-row" data-job-no="{{ $job->job_no }}">
                                        ...
                                        <td>
                                            @if ($job->cost_rate_value != 0)
                                                {{ number_format($job->cost_kba_thb / $job->cost_rate_value, 2) }}
                                            @endif
                                        </td>
                                        <td>{{ number_format($job->cost_kba_thb, 2) }}</td>
                                    </tr>
                                    @php
                                        // Accumulate totals only if cost_rate_value is not zero
                                        if ($job->cost_rate_value != 0) {
                                            $total_usd += $job->cost_kba_thb / $job->cost_rate_value;
                                        }
                                        $total_thb += $job->cost_kba_thb;
                                        $item_count++;
                                    @endphp
                                @endif
                            @endforeach
